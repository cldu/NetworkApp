...
export class PhotoEditorComponent implements OnInit {
  @Input() photos: Photo[];
  @Output() getProfilePhoto = new EventEmitter<string>();
  ...
  setProfilePhoto(photo: Photo) {
    this.userService.setProfilePhoto(this.authService.decodedToken.nameid, photo.id).subscribe(() => {
      this.currentProfile = this.photos.filter(p => p.isProfilePhoto === true)[0];
      this.currentProfile.isProfilePhoto = false;
